...
use codex_cli::run_newapi_login;
...
#[clap(
    author,
    version,
    // If a sub鈥慶ommand is given, ignore requirements of the default args.
    subcommand_negates_reqs = true,
    // The executable is sometimes invoked via a platform鈥憇pecific name like
...
struct LoginCommand {
    ...
    /// Connect to a New API (newapi) relay and select/create an API key.
    #[arg(long = "newapi")]
    newapi: bool,

    /// Base URL of the New API relay (omit any /v1 suffix).
    #[arg(long = "newapi-base-url", value_name = "URL")]
    newapi_base_url: Option<String>,

    /// Skip admin login and paste a New API access token directly.
    #[arg(long = "newapi-with-token")]
    newapi_with_token: bool,
...
fn run_update_action(action: UpdateAction) -> anyhow::Result<()> {
    ...
    println!("\n馃帀 Update ran successfully! Please restart Codex.");
    Ok(())
}
            ...
            match login_cli.action {
                ...
                None => {
                    if login_cli.with_api_key && login_cli.with_access_token {
                        ...
                        std::process::exit(1);
                    } else if login_cli.newapi || login_cli.newapi_with_token {
                        run_newapi_login(
                            login_cli.config_overrides,
                            login_cli.newapi_base_url,
                            login_cli.newapi_with_token,
                        )
                        .await;
